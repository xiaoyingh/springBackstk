	...
	$('#adglsave').click(function(){
		...
		$.ajax({
			...
			success : function(msg) {
				...
				if (msg.id == 1) {
					alert("添加成功");
					window.location.href=''+path+'/selvideoxiao?id='+msg.sid+'&page='+bring;
...
// alert('正常操作');
	...
	$('tbody .active').children().each(function(){
		//var str = ' <span>排序：</span><input class="easyui-numberspinner" value="1000" data-options="increment:100" style="width:120px;" />';
	//	$("#xiugai").html(str);
		...
		<div class="lis title" style="height: 100px;">
			...
			<span style="float: left;width: 100px;text-align: right;">视频文件：</span> <input type="text" name="" id="glvideourl" value="`+$('tbody .active').children().eq(2).html()+`" style="float: left;width: 365px;" />
			</div>
...
function deletecogl() {
	...
	$.ajax({
		...
		success : function(msg) {
			...
			if (msg.id == 1) {
				alert("删除成功");
				window.location.href=''+path+'/selvideoxiao?id='+msg.sid+'';
