	...
	<div style="padding: 5px 5px 0 5px;width: 100%;height: 100%;">
		<div class="first-head">
			...
			<a href="javascript:;" class="gl-btn easyui-linkbutton"
				onclick="addTab('iframepage/video/gl/glmp.jsp')" ><span
		...
		<div class="modal-body">
			<div class="panel-wrap">
				<div class="lis ">	
					<span>分类：</span>	
					<select name="" id="ddl">
						<c:forEach items="${typeb }" var="bs">
							<option   style="font-size: 16px;font-weight: 700;color: royalblue;" 
							first="${bs.typeName }"; second=""; third=""; firstid="${bs.ID }"; secondid=""; thirdid="";
							>${bs.typeName }</option>
							
							<c:forEach items="${asd }" var="bsd">
								<c:if test="${bsd.parentID == bs.ID }">
							 		<option first="${bs.typeName }"; second="${bsd.typeName }"; third=""; firstid="${bs.ID }"; secondid="${bsd.ID }"; thirdid="";>
							 		<span class="glyphicon glyphicon-list"></span>
									${bs.typeName }--${bsd.typeName }
							 		</option>
								</c:if>
							</c:forEach>
						</c:forEach>
...
<!--  
								<script type="text/javascript">
									$(document)
											.ready(
													function() {
														function ajaxFileUploadbg() {
															var formData = new FormData();
															formData
																	.append(
																			'file',
																			$("#file_uploadbg")[0].files[0]); //将文件转成二进制形式  
															$
																	.ajax({
																		type : "post",
...
-->
						...
						<div class="lis sfsf" style="display:none;">
							<span></span> <input type="hidden" value="0" name="adIsPrice" />
							<input type="hidden" value="1" name="adIsPrice" />
						</div>

						<div class="lis sfsf" style="display:none;">
							<span></span> <input type="hidden" value="0" name="adfupin" />
							<input type="hidden" value="1" name="adfupin" />
						</div>

						<div class="lis sfsf" style="display:none;">
							<span></span> <input type="hidden" value="0" name="aspubli" />
							<input type="hidden" value="1" name="aspubli" />
						</div>
						...
						<div class="lis pay">
							<span>作者：</span> <input type="text" id="writer" />
						</div>
					</div>
				</div>
...
var path=localhostPaht+projectName;
        ...
        function xxvideo(obj) {
          ...
          //alert(page);
          window.location.href='<%=path%>/mpseries?id=189&page='+page;
...
<script type="text/javascript" src="<%=path%>/js/iframepage/mpseriespublic.js"></script>
